...
class MediTalkProApp(ctk.CTk):
    ...
    def send_whatsapp_desktop(self):
        ...
        try:
            contact_number = self.listen_for_response("What is the full phone number, including the country code?")
            if not contact_number: return

            message = self.listen_for_response("What is the message?")
            if not message: return
            
            # Reformat number for pywhatkit
            contact_number = '+' + ''.join(filter(str.isdigit, contact_number))

            self.speak(f"Sending '{message}' to {contact_number}. Please wait.")
            
            # This function uses the desktop app if available
            pywhatkit.sendwhatmsg_to_group_instantly(contact_number, message, wait_time=15)
            pyautogui.press('enter')
            self.speak("The message has been sent.")

        except Exception as e:
            self.speak("I couldn't send the message. Please make sure WhatsApp is installed and you are logged in.")
    ...
    def process_command(self, command):
        ...
        if "open notepad" in command:
            ...
            # Assumes 'code' is in the system's PATH
            os.system("code")
